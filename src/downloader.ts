...
export function nodeHttpRequestAsync(
    options: HttpRequestOptions,
    validate?: (u: http.RequestOptions) => void
...
export function requestAsync(
    ...
    return nodeHttpRequestAsync(options).then<HttpResponse>(resp => {
        if (
            resp.statusCode != 200 &&
            resp.statusCode != 304 &&
